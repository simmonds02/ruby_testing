...
describe CaesarBreaker do
  ...
  describe '#create_decrypted_messages' do
    ...
    it 'sends translate 26 times' do
      ...
      expect(phrase.create_decrypted_messages).to eql(26)
    end
  end
  ...
  describe '#save_decrypted_messages' do
    ...
    context 'when the directory exists' do
      before do
        ...
        allow(Dir).to receive(:mkdir)
        allow(File).to receive(:open).once
        allow(phrase).to receive(:save_to_yaml)
        allow(phrase).to receive(:display_file_location)
        allow(phrase).to receive(:create_filename)
      end
