...
describe Drink do
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:my_full_drink) {described_class.new('coffee', 32)}

      # remove the 'x' before running this test
      it 'is full' do
        expect(my_full_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:my_not_full_drink) {described_class.new('coffee', 8)}

      # remove the 'x' before running this test
      it 'is not full' do
        expect(my_not_full_drink).not_to be_full
      end
    end
  end
end
